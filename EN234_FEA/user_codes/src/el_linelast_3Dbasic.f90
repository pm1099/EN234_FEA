...
!==========================SUBROUTINE el_linelast_3d_Bbar ==============================
subroutine el_linelast_3d_Bbar(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    n_properties, element_properties, element_coords, length_coord_array, &                      ! Input variables
    dof_increment, dof_total, length_dof_array, &                                                ! Input variables
    n_state_variables, initial_state_variables, &                                                ! Input variables
    updated_state_variables,element_stiffness,element_residual, fail)      ! Output variables                          ! Output variables
    ...
    real (prec)  ::  D(6,6)                            ! stress = D*(strain+dstrain)  (NOTE FACTOR OF 2 in shear strain)
    real (prec)  ::  B(6,length_dof_array)             ! strain = B*(dof_total+dof_increment)
    real (prec)  ::  Bbar(6,length_dof_array)          ! The Bbar matrix
    real (prec)  ::  Bcorr(6,length_dof_array)         ! Bbar=B+(1/3)*Bcorr
    real (prec)  ::  dxidx(3,3), determinant           ! Jacobian inverse and determinant
    real (prec)  ::  x(3,length_coord_array/3)         ! Re-shaped coordinate array x(i,a) is ith coord of ath node
    real (prec)  :: E, xnu, D44, D11, D12              ! Material properties
    real (prec) :: dNdx_vec(1,1:3*n_nodes),dNbardx_vec(1,1:3*n_nodes) ! Reshaped matrices
    real (prec):: el_vol
    !
    !     Subroutine to compute element stiffness matrix and residual force vector for 3D linear elastic elements
    !     El props are:
    ...
    do kint = 1, n_points
       ...
       dNbardx=0.d0
        ...
        dNdx_vec=reshape(transpose(dNdx),(/1,3*n_nodes/))
        dNbardx_vec=reshape(transpose(dNbardx(1:n_nodes,1:3)),(/1,3*n_nodes/))
        Bcorr(1,1:3*n_nodes)=-dNdx_vec(1,1:3*n_nodes)+dNbardx_vec(1,1:3*n_nodes)
        Bcorr(2,1:3*n_nodes)=-dNdx_vec(1,1:3*n_nodes)+dNbardx_vec(1,1:3*n_nodes)
        Bcorr(3,1:3*n_nodes)=-dNdx_vec(1,1:3*n_nodes)+dNbardx_vec(1,1:3*n_nodes)
        Bcorr(4,1:3*n_nodes)=0.d0
        Bcorr(5,1:3*n_nodes)=0.d0
        Bcorr(6,1:3*n_nodes)=0.d0
        Bbar(1:6,1:3*n_nodes)=B(1:6,1:3*n_nodes)+(1/3)*(Bcorr(1:6,1:3*n_nodes))
    ...
    return
end subroutine el_linelast_3d_Bbar



subroutine fieldvars_linelast_3d_Bbar(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    n_properties, element_properties,element_coords,length_coord_array, &                                ! Input variables
    dof_increment, dof_total, length_dof_array,  &                                                      ! Input variables
    n_state_variables, initial_state_variables,updated_state_variables, &                               ! Input variables
    n_field_variables,field_variable_names, &                                                           ! Field variable definition
    ...
    return
end subroutine fieldvars_linelast_3d_Bbar
