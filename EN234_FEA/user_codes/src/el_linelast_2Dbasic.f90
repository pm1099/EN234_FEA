...
subroutine el_linelast_2dbasic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    ...
    real (prec)  ::  Bcorr(3,length_dof_array)         ! Correction matrix for Bbar
    real (prec)  ::  Bbar(3,length_dof_array)          ! Bbar=B+0.5*Bcorr
    ...
    if (element_identifier==101) then   ! Basic 2D Element Identifier: 101
    !     --  Loop over integration points
    ...
    else if (element_identifier==102) then  !B-bar element
        ...
        do kint=1,n_points
            ...
            Bcorr=0.d0
            Bcorr(1,1:2*n_nodes-1:2)=dNbardx(1:n_nodes,1)-dNdx(1:n_nodes,1)
            Bcorr(1,2:2*n_nodes:2)=dNbardx(1:n_nodes,2)-dNdx(1:n_nodes,2)
            Bcorr(2,1:2*n_nodes-1:2)=dNbardx(1:n_nodes,1)-dNdx(1:n_nodes,1)
            Bcorr(2,2:2*n_nodes:2)=dNbardx(1:n_nodes,2)-dNdx(1:n_nodes,2)
...
subroutine fieldvars_linelast_2dbasic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    ...
    real (prec)  ::  D(3,3)                            ! stress = D*(strain+dstrain)  (NOTE FACTOR OF 2 in shear strain)
    real (prec)  ::  B(3,length_dof_array)             ! strain = B*(dof_total+dof_increment)
    real (prec)  ::  Bcorr(3,length_dof_array)         ! Correction Matrix
    real (prec)  ::  Bbar(3,length_dof_array)          ! Babar=B+0.5*Bcorr
    real (prec)  ::  dxidx(2,2), determinant           ! Jacobian inverse and determinant
    real (prec)  ::  x(2,length_coord_array/2)         ! Re-shaped coordinate array x(i,a) is ith coord of ath node
        ...
        do kint = 1, n_points
            ...
            do k = 1,n_field_variables
                if (strcmp(field_variable_names(k),'S11',3) ) then
                    ...
                    nodal_fieldvariables(k,1:n_nodes) = nodal_fieldvariables(k,1:n_nodes) &
                    + s33*N(1:n_nodes)*determinant*w(kint)
