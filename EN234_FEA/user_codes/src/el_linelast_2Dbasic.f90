...
subroutine el_linelast_2dbasic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    ...
    if (n_nodes == 6) n_points = 4  !Traingular elements
    if (n_nodes == 8) n_points = 9  !Quadrilateral elements
...
subroutine fieldvars_linelast_2dbasic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    ...
    real (prec)  ::  x(2,length_coord_array/2)         ! Re-shaped coordinate array x(i,a) is ith coord of ath node
    real (prec)  :: E, xnu,d11,d33,d12             ! Material properties
...
!==========================SUBROUTINE el_linelast_2dbasic ==============================
subroutine el_linelast_2d_Bbar(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    n_properties, element_properties,element_coords, length_coord_array, &                                                                ! Input variables
    dof_increment, dof_total, length_dof_array, &                                                ! Input variables
    n_state_variables, initial_state_variables, &                                                ! Input variables
    updated_state_variables,element_stiffness,element_residual, fail)      ! Output variables                          ! Output variables
    ...
    ! Local Variables
    integer      :: n_points,kint,m,a,i

    real (prec)  ::  strain(3), dstrain(3)             ! Strain vector contains [e11, e22, 2e12]
    real (prec)  ::  stress(3)                         ! Stress vector contains [s11, s22, s12]
    real (prec)  ::  D(3,3)                            ! stress = D*(strain+dstrain)  (NOTE FACTOR OF 2 in shear strain)
    real (prec)  ::  B(3,length_dof_array)             ! strain = B*(dof_total+dof_increment)
    real (prec)  ::  Bbar(3,length_dof_array)          ! The Bbar matrix
    real (prec)  ::  Bcorr(3,length_dof_array)         ! Bbar=B+(1/2)*Bcorr
    real (prec)  ::  dxidx(2,2), determinant           ! Jacobian inverse and determinant
    real (prec)  ::  x(2,length_coord_array/2)         ! Re-shaped coordinate array x(i,a) is ith coord of ath node
    real (prec)  ::  E, xnu, d33, d11, d12                            ! Material properties
    real (prec) :: dNdx_vec(1,1:2*n_nodes),dNbardx_vec(1,1:2*n_nodes) ! Reshaped matrices
    real (prec) :: el_vol
    !
    !     Subroutine to compute element stiffness matrix and residual force vector for 2D linear elastic elements
    !     El props are:
  ...
  ! Finding dNbardx
       do m=1,n_points
       call calculate_shapefunctions(xi(1:2,m),n_nodes,N,dNdxi)
        dxdxi = matmul(x(1:2,1:n_nodes),dNdxi(1:n_nodes,1:2))
        call invert_small(dxdxi,dxidx,determinant)
        dNdx(1:n_nodes,1:2) = matmul(dNdxi,dxidx(1:2,1:2))
        el_vol=0.d0
        dNbardx=0.d0
        do a=1,n_nodes
            do i=1,2
                dNbardx(a,i)=dNbardx(a,i)+dNdx(a,i)*w(m)*determinant-dNdx(a,i)
            end do
        end do
    ...
    do kint = 1, n_points
      ...
      dNdx_vec=reshape(transpose(dNdx),(/1,2*n_nodes/))
       dNbardx_vec=reshape(transpose(dNbardx(1:n_nodes,1:2)),(/1,2*n_nodes/))
        Bcorr(1,1:2*n_nodes)=dNbardx(1,1:2*n_nodes)
        Bcorr(2,1:2*n_nodes)=dNbardx(2,1:2*n_nodes)
        Bcorr(3,1:2*n_nodes)=0.d0
        Bbar(1:3,1:2*n_nodes)=B(1:3,1:2*n_nodes)+0.5d0*Bcorr(1:3,1:2*n_nodes)
    ...
    return
end subroutine el_linelast_2d_Bbar


!==========================SUBROUTINE fieldvars_linelast_2dbasic ==============================
subroutine fieldvars_linelast_2d_Bbar(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    n_properties, element_properties,element_coords,length_coord_array, &                                ! Input variables
    dof_increment, dof_total, length_dof_array,  &                                                      ! Input variables
    n_state_variables, initial_state_variables,updated_state_variables, &                               ! Input variables
    n_field_variables,field_variable_names, &                                                           ! Field variable definition
...
! Finding dNbardx
    ...
    return
end subroutine fieldvars_linelast_2d_Bbar
